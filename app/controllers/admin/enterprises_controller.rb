...
module Admin
  class EnterprisesController < Admin::ResourceController
    ...
    def load_methods_and_fees
      ...
      @payment_methods = Spree::PaymentMethod.includes(:calculator).managed_by(spree_current_user).to_a.sort_by! do |pm|
        [(enterprise_payment_methods.include? pm) ? 0 : 1, pm.name]
      end
      @shipping_methods = Spree::ShippingMethod.includes(:calculator).managed_by(spree_current_user).to_a.sort_by! do |sm|
